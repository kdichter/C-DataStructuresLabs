...
struct ListNode
{
  ...
  unhook_range (ListNode* begin, ListNode* end)
  {
    ...
    begin->prev->next = end->next; //from .next
    end->next->prev = begin->prev; //from .prev
  }
...
public: /* should be private, but public for testing */
  ...
  transfer (iterator pos, iterator first, iterator last)
  {
    ...
    if (first != last) //distance (first, last) != 0
    {
      //   if not empty, unhook_range the inclusive range [first, std::prev(last)],
      //   and hook_range that range before pos
      --last;
      Node* newFirst = first.m_nodePtr;
      Node* newLast = last.m_nodePtr;
      Node* newPos = pos.m_nodePtr;

      Node::unhook_range (newFirst, newLast);
      newPos->Node::hook_range (newFirst, newLast);
...
public:
  ...
  List () : m_header (), m_size (0)
  {
    ...
    m_header.next = &m_header; //&m_header;
  ...
  operator= (const List& other)
  {
    ...
    if (this != &other)
    {
      m_size = other.size ();
      clear ();
      //copy (other.begin (), other.end (), m_header);
      auto it = other.begin ();
      for (size_t i = 0; i < other.size (); ++i)
      {
        push_back (it.m_nodePtr->data);
        ++it;
      }
    }
    return *this;
